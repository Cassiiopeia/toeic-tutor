...
5. **공식 출제범위 8분야를 돌려 쓴다.** 아래는 한국토익위원회가 공개한 실제 출제범위다 — 조수가 임의로 만든 목록이 아니다. (Part 7 지문 종류 9가지와는 다른 축이다 — 범위는 "무슨 업무 이야기인가", 지문 종류는 "어떤 문서 형식인가".)

   | 분야 | 세부 |
   |---|---|
   | 전문적인 비즈니스 | 계약 · 협상 · 마케팅 · 세일즈 · 비즈니스 계획 · 회의 |
   | 제조 | 공장 관리 · 조립라인 · 품질관리 |
   | 금융과 예산 | 은행 · 투자 · 세금 · 회계 · 청구 |
   | 개발 | 연구 · 제품개발 |
   | 사무실 | 임원회의 · 위원회의 · 편지 · 메모 · 전화 · 팩스 · 이메일 · 사무 장비와 가구 |
   | 인사 | 구인 · 채용 · 퇴직 · 급여 · 승진 · 취업 지원과 자기소개 |
   | 주택 · 기업 · 부동산 | 건축 · 설계서 · 구입과 임대 · 전기와 가스 서비스 |
   | 여행 | 기차 · 비행기 · 택시 · 버스 · 배 · 유람선 · 티켓 · 일정 · 역과 공항 안내 · 렌터카 · 호텔 · 예약 · 연기와 취소 |

5b. **공식 출제기준을 지킨다.** 이걸 어기면 실제 시험과 결이 다른 문제가 나온다.

   - 미국 영어에만 쓰이는 어휘 · 문법 · 관용어를 쓰지 않는다
   - 특정 문화권에만 통하거나 일부 응시자에게 생소한 상황을 쓰지 않는다
   - **여러 나라 사람 이름을 고르게 등장시킨다** — 같은 세션에서 이름이 한쪽 문화로 쏠리지 않게 한다
   - 특정 직업 분야에만 해당되는 상황을 쓰지 않는다
   - 문화와 성에 대한 편견이 없게 한다
...
- 단일 지문(2~4문항)은 지문이 하나뿐이라 연계가 불가능하다 — 세부 · 추론 · NOT · 동의어 · 삽입위치 · 목적 중에서 채우고, NOT/EXCEPT 는 지문당 최대 1개. 다만 그 지문이 문자 메시지 체인이나 온라인 채팅이면 의도 문제(`의도`) 1개를 반드시 넣는다(`rc/format.md` 「지문 9종」).
...
세트 하나가 훈련 모드의 "문제 하나"다(지문 + 문항 2~5개). 답은 문항별로 받고 문항별로 판정한다.
...
- Part 5 10문제 (구문을 뺀 6개 유형 — 품사·동사·준동사·전접·대명·어휘 — 을 고르게 순환, 같은 유형 3연속 금지. 구문은 실전 세트에서만 1문제 이하로 섞는다)
- Part 6 4문항 (1지문)
