...
**조수가 채점하지 않는다. 자습.** Q1-2 는 지문을 그대로 읽는 문항이라 채점할 내용·구조가 없고,
채점 잣대는 발음·억양·강세인데 이 레포는 타이핑이라 그걸 볼 수 없다 (`speaking/rubric.md` 참고).
아래 표시법대로 소리 내어 읽고, 자기 점검 목록으로 스스로 확인한다.

## 표시법

| 표시 | 기호 | 의미 |
|---|---|---|
| 강세 | **굵게** | 힘주어 읽는 단어 — 새 정보·숫자·고유명사·대조되는 단어 |
| 끊어읽기 | `/` | 숨 쉬며 잠깐 멈추는 자리 — 구·절 경계 |
| 연음 | `‿` | 앞 단어 끝 자음과 뒷 단어 첫 모음이 이어져 들리는 자리 |
| 올림/내림 | `↗` `↘` | 구·문장 끝의 억양 — 열거 중간·의문문은 ↗, 문장 마무리는 ↘ |

## 지문 예시 세 개

각 40~60단어. 표시법대로 한 줄씩 소리 내어 읽는다. **실제 토익 지문이 아닌 이 레포가 만든 예시**다.
...
읽고 나서 다섯 개를 스스로 확인한다. 셋 미만이면 다시 읽는다.
